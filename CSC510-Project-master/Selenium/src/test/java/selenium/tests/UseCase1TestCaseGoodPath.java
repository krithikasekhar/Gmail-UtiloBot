...
public class UseCase1TestCaseGoodPath {
	...
	@Test
	public void useCase1TestCaseGoodPath()
	{
		...
		if(labelText.contains("Today") == true){
			...
			String checkString = "Commit Summary";
			int lastIndex = 0;
			int count = 0;
			...
			if(count == 4){
				actions.sendKeys("close pull request with pull=2 repo=test3 owner=asingh26");
				actions.sendKeys(Keys.RETURN);
				actions.build().perform();
				...
				checkString = "Given request is closed successfully";
				assertTrue(replyText.contains(checkString));
				
				if(replyText.contains(checkString)){
					actions.sendKeys("merge pull request with pull=2 repo=test3 owner=asingh26");
					actions.sendKeys(Keys.RETURN);
					actions.build().perform();
					...
					checkString = "Pull Request is not mergeable";
					assertTrue(replyText.contains(checkString));
				}
			}
		}
